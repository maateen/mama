...
# The interface class to start recording and communicating with Microsoft
class interface():
    """
    ...
    def sendto(self, client_id, api_key):
        ...
        params = urllib.parse.urlencode(
            {'grant_type': 'client_credentials', 'client_id': clientId, 'client_secret': clientSecret, 'scope': Host})
        ...
        if response.status == 200:
            ...
            if response.status == 200:
                ...
                os.system('echo "' + text['speechbox-root']['results']['result'][
                    'name'] + '" > /tmp/mama/mama_result_' + self.PID)
